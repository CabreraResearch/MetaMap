...
"use strict";var riot=require("riot"),moment=require("moment"),NProgress=window.NProgress,_=require("lodash"),$=require("jquery"),Dropzone=require("dropzone"),Papa=require("papaparse");require("datatables"),require("datatables-bootstrap3-plugin");var CONSTANTS=require("../../constants/constants"),raw=require("../components/raw"),TrainingMix=require("../mixins/training-mix"),html='\n<table class="table table-striped table-bordered table-hover" id="{tableId}">\n    <thead>\n        <tr>\n            <th each="{columns}">{name}</th>\n        </tr>\n    </thead>\n    <tbody>\n        <tr if="{ data }" each="{ data }" class="odd gradeX">\n            <td>\n                <a class="btn btn-sm red" onclick="{ parent.onStart }">Start <i class="fa fa-play"></i></a>\n            </td>\n            <td class="{ meta_editable: parent.editable}" data-pk="{ id }" data-name="name" data-title="Edit Course Name" style="vertical-align: middle;">{ name }</td>\n            <td class="{ meta_editable: parent.editable}" data-pk="{ id }" data-name="description" data-title="Edit Course Description" style="vertical-align: middle;">{ description }</td>\n            <td if="{ parent.user.isAdmin }" >\n                <form  method="{ parent.onUpload }" url="/noupload" class="dropzone" id="training_upload_{ id }"></form>\n            </td>\n        </tr>\n    </tbody>\n</table>\n';module.exports=riot.tag(CONSTANTS.TAGS.ALL_COURSES,html,function(e){var t=this;this.mixin(TrainingMix);var a=require("../../../MetaMap.js");this.user=a.User,this.data=[],this.editable=!1,this.tableId="all_courses",this.dropzones={},this.columns=[{name:"Action",orderable:!1,width:"120px"},{name:"Name",orderable:!0},{name:"Description",orderable:!0}],this.user.isAdmin&&this.columns.push({name:"Upload Training",orderable:!1}),this.onStart=function(e){a.Router.to("trainings/"+e.item.id)},this.onUpload=function(e){console.log(e)},this.initDropzone=function(e){try{!function(){var a=t;a.dropzones[e]=a.dropzones[e]||new Dropzone("#training_upload_"+e,{url:"/noup",paramName:"file",maxFiles:0,addRemoveLinks:!0,autoProcessQueue:!1,dictMaxFilesExceeded:"One training at a time!",acceptedFiles:".csv",init:function(){var t=this;t.on("drop",function(e){if(t.files.length>0||!e.dataTransfer.files[0].name.endsWith(".csv"))throw new Error("Only 1 CSV file at a time.")}),t.on("addedfile",function(n){Papa.parse(n,{complete:function(n,i){a.saveTraining(e,n.data),t.removeAllFiles(!0),window.alert("Your training has been successfully parsed!")}})})}})}()}catch(a){console.log(a)}},this.buildTable=function(){try{t.table&&(t.table.find(".meta_editable").editable("destroy"),t.dataTable.destroy()),t.table=$(document.getElementById([t.tableId])),t.dataTable=t.table.DataTable({columns:t.columns});var e=t.table.parent().parent().parent().find("#"+t.tableId+"_table_wrapper");e.find(".dataTables_length select").addClass("form-control input-xsmall input-inline"),t.table.find(".meta_editable").editable({unsavedclass:null}).on("save",function(e,t){if(this.dataset&&this.dataset.pk){var n=this.dataset.pk;a.MetaFire.setData(t.newValue,CONSTANTS.ROUTES.COURSE_LIST+"/"+n+"/"+this.dataset.name)}return!0})}catch(n){}finally{NProgress.done()}},this.on("mount",function(){t.editable=t.user.isAdmin});var n=_.once(function(){a.MetaFire.on(CONSTANTS.ROUTES.COURSE_LIST,function(e){t.data=_.map(e,function(e,t){return e.id=t,e.created_at=moment(new Date(e.created_at)).format("YYYY-MM-DD"),e}),t.data&&(t.update(),t.buildTable(0,t.data),_.each(t.data,function(e){t.initDropzone(e.id)}))})});this.on("update",function(){n()})});

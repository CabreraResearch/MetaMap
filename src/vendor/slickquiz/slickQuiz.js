    ...
    $.slickQuiz = function(element, options) {
        ...
        var internal = {method: {
            ...
            getKey: function (notches) { // returns [], notches >= 1
                var key = [];
                for (let i=0; i<notches; i++) key[i] = $.Deferred ();
                return key;
            },
        ...
        plugin.method = {
            ...
            setupQuiz: function(options) { // use 'options' object to pass args
                ...
                if (plugin.config.tryAgainText && plugin.config.tryAgainText !== '') {
                    $quizResultsCopy.append('<p><button class="btn red ' + tryAgainClass + '" href="#">' + plugin.config.tryAgainText + '</button></p>');
                }
                ...
                for (let i in questions) {
                    if (questions.hasOwnProperty(i)) {
                        ...
                        var answerHTML = $('<ul class="' + answersClass + ' list-group"></ul>');
                        ...
                        for (let i in answers) {
                            if (answers.hasOwnProperty(i)) {
                                ...
                                let optionId = inputName + '_' + i.toString()

                                // If question has >1 true answers and is not a select any, use checkboxes; otherwise, radios
                                var input = '<input id="' + optionId + '" name="' + inputName +
                                            '" type="' + inputType + '" " class="slickAnswerButton" /> ';

                                var optionLabel = '<span id="' + optionId + '_answer">' + answer.option + '</span>';

                                var answerContent = $('<li class="list-group-item" style="text-align: left; padding-left: 20px;"></li>')
                                    .append(input)
                                    .append(optionLabel);
                        ...
                        if (plugin.config.backButtonText && plugin.config.backButtonText !== '') {
                            questionHTML.append('<button href="#" class="left btn ' + backToQuestionClass + '">' + plugin.config.backButtonText + '</button>');
                        }
                        ...
                        if (!plugin.config.perQuestionResponseMessaging) {
                            questionHTML.append('<button href="#" class="btn red right ' + nextQuestionClass + ' ' + checkAnswerClass + '">' + nextText + '</button>');
                        } else {
                            questionHTML.append('<button href="#" class="btn red right ' + nextQuestionClass + '">' + nextText + '</button>');
                            questionHTML.append('<button href="#" class="btn green right ' + checkAnswerClass + '">' + plugin.config.checkAnswerText + '</button>');
                        }
                ...
                for (let i in answers) {
                    if (answers.hasOwnProperty(i)) {
                        ...
                        if (answer.correct) {
                            ...
                            trueAnswerVals.push(answer.option)
            ...
            completeQuiz: function(options) {
                ...
                internal.method.turnKeyAndGo (key, options && options.callback ? options.callback : function () {}, results);
